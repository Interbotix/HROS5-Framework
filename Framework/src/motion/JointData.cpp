...
//    SetEnable(ID_R_SHOULDER_PITCH_RIG,  enable, exclusive);
//    SetEnable(ID_R_SHOULDER_ROLL_RIG,   enable, exclusive);
//    SetEnable(ID_R_ELBOW_RIG,           enable, exclusive);
...
//    SetEnable(ID_L_SHOULDER_PITCH_RIG,  enable, exclusive);
//    SetEnable(ID_L_SHOULDER_ROLL_RIG,   enable, exclusive);
//    SetEnable(ID_L_ELBOW_RIG,           enable, exclusive);
...
{
    if (value < MXDXL::MIN_VALUE)
        value = MXDXL::MIN_VALUE;
    else if (value >= MXDXL::MAX_VALUE)
        value = MXDXL::MAX_VALUE;

    m_Value[id] = value;
    m_Angle[id] = MXDXL::Value2Angle(value);
}
...
{
    if (angle < MXDXL::MIN_ANGLE)
        angle = MXDXL::MIN_ANGLE;
    else if (angle > MXDXL::MAX_ANGLE)
        angle = MXDXL::MAX_ANGLE;

    m_Angle[id] = angle;
    m_Value[id] = MXDXL::Angle2Value(angle);
}
