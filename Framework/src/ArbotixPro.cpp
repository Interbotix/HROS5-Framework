...
BulkReadData::BulkReadData() :
	...
	error(-1)
{
	for (int i = 0; i < MXDXL::MAXNUM_ADDRESS; i++)
		table[i] = 0;
}
...
//            m_BulkReadTxPacket[PARAMETER+3*number+3] = MXDXL::P_PRESENT_POSITION_L; // start address
//            number++;
//        }
//    }
					...
					if (MotionStatus::m_CurrentJoints.GetEnable(id))
						{
							...
							m_BulkReadTxPacket[PARAMETER + 3 * number + 2] = id; // id
							m_BulkReadTxPacket[PARAMETER + 3 * number + 3] = MXDXL::P_PRESENT_TEMPERATURE; // start address
							number++;
						}
				}
		}
