...
//enable head/head tracking
    Head::GetInstance()->m_Joint.SetEnableHeadOnly(true, true);
//enable walking headless (head belongs to Head class)
    Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
	}
		
//check if in Auto modes, if not, shut down when X is pressed.		
...
// Initialize walking gait engine
		//start		
...
// if in sitting (not squat) position, play special standup routine
			if(lastMode == SITTING)
				Action::GetInstance()->Start(8); //50
				
			//else if(lastMode == STAIRS && MotionStatus::FALLEN != STANDUP)
				//Action::GetInstance()->Start(8); //8
//if standing or squatting (only other two positions) stand up slowly				
			else
      ...
      Head::GetInstance()->m_Joint.SetEnableHeadOnly(true);
			while(command.buttons&BUT_LT != 0) usleep(8000);			
			}
