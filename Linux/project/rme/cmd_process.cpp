	...
	for (int id = 0; id < 31; id++)
		{
			if (id >= JointData::ID_MIN && id <= JointData::ID_MAX)
				{
					if (arbotixpro->ReadByte(id, MXDXL::P_TORQUE_ENABLE, &value, 0) == ArbotixPro::SUCCESS)
						{
							if (value == 1)
								{
									if (arbotixpro->ReadWord(id, MXDXL::P_GOAL_POSITION_L, &value, 0) == ArbotixPro::SUCCESS)
	...
	if ( col == STP7_COL )
		{
			if ( row == PAUSE_ROW )
				...
				{
					if (value >= 0 && value <= MXDXL::MAX_VALUE)
						{
							if (!(Step.position[row + 1] & Action::INVALID_BIT_MASK) && !(Step.position[row + 1] & Action::TORQUE_OFF_BIT_MASK))
								{
									int error;
									if (arbotixpro->WriteWord(row + 1, MXDXL::P_GOAL_POSITION_L, value, &error) == ArbotixPro::SUCCESS)
										{
...
void ToggleTorque(ArbotixPro *arbotixpro)
{
	...
	if (Step.position[id] & Action::TORQUE_OFF_BIT_MASK)
		{
			if (arbotixpro->WriteByte(id, MXDXL::P_TORQUE_ENABLE, 1, 0) != ArbotixPro::SUCCESS)
				return;

			int value;
			if (arbotixpro->ReadWord(id, MXDXL::P_PRESENT_POSITION_L, &value, 0) != ArbotixPro::SUCCESS)
				return;
		...
		{
			if (arbotixpro->WriteByte(id, MXDXL::P_TORQUE_ENABLE, 0, 0) != ArbotixPro::SUCCESS)
				return;
...
void OnOffCmd(ArbotixPro *arbotixpro, bool on, int num_param, int *list, char lists[30][10])
{
	...
	if (num_param == 0)
		{
			for (int id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
				arbotixpro->WriteByte(id, MXDXL::P_TORQUE_ENABLE, (int)on, 0);
		}
	else
		{
							...
							if (token != NULL)
							...
							else
								{
									if (strcmp(token1, "rl") == 0)
										{
											for (id = 0; id < 6; id++ )
												arbotixpro->WriteByte(rl[id], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
										}
									else if (strcmp(token1, "ll") == 0)
										{
											for (id = 0; id < 6; id++ )
												arbotixpro->WriteByte(ll[id], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
										}
									else if (strcmp(token1, "ra") == 0)
										{
											for (id = 0; id < 6; id++ )
												arbotixpro->WriteByte(ra[id], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
										}
									else if (strcmp(token1, "la") == 0)
										{
											for (id = 0; id < 6; id++ )
												arbotixpro->WriteByte(la[id], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
										}
									else if (strcmp(token1, "h") == 0)
										{
											for (id = 0; id < 3; id++ )
												arbotixpro->WriteByte(h[id], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
										}
									else if (list[i] >= JointData::ID_MIN && list[i] <= JointData::ID_MAX)
										arbotixpro->WriteByte(list[i], MXDXL::P_TORQUE_ENABLE, (int)on, 0);
								}
						}
				}
		}
	...
	for (id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
		{
			...
			if (arbotixpro->ReadWord(id, MXDXL::P_PRESENT_POSITION_L, &wStartPosition, 0) != ArbotixPro::SUCCESS)
				{
