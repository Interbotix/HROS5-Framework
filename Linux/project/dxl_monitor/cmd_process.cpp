...
void Dump(ArbotixPro *arbotixpro, int id)
{
	...
	if (id == ArbotixPro::ID_CM) // Sub board
		...
		{
			if (arbotixpro->ReadTable(id, MXDXL::P_MODEL_NUMBER_L, MXDXL::P_PUNCH_H, &table[MXDXL::P_MODEL_NUMBER_L], 0) != ArbotixPro::SUCCESS)
				{
					printf(" Can not read table!\n");
					return;
				}
			...
			printf( "\n" );
			printf( " [EEPROM AREA]\n" );
			addr = MXDXL::P_MODEL_NUMBER_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " MODEL_NUMBER            (R) [%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_VERSION; value = table[addr];
			printf( " VERSION                 (R) [%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_ID; value = table[addr];
			printf( " ID                     (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_BAUD_RATE; value = table[addr];
			printf( " BAUD_RATE              (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_RETURN_DELAY_TIME; value = table[addr];
			printf( " RETURN_DELAY_TIME      (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_CW_ANGLE_LIMIT_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " CW_ANGLE_LIMIT         (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_CCW_ANGLE_LIMIT_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " CCW_ANGLE_LIMIT        (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_HIGH_LIMIT_TEMPERATURE; value = table[addr];
			printf( " HIGH_LIMIT_TEMPERATURE (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_LOW_LIMIT_VOLTAGE; value = table[addr];
			printf( " LOW_LIMIT_VOLTAGE      (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_HIGH_LIMIT_VOLTAGE; value = table[addr];
			printf( " HIGH_LIMIT_VOLTAGE     (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_MAX_TORQUE_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " MAX_TORQUE             (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_RETURN_LEVEL; value = table[addr];
			printf( " RETURN_LEVEL           (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_ALARM_LED; value = table[addr];
			printf( " ALARM_LED              (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_ALARM_SHUTDOWN; value = table[addr];
			printf( " ALARM_SHUTDOWN         (R/W)[%.3d]:%5d\n", addr, value);
			printf( "\n" );
			printf( " [RAM AREA]\n" );
			addr = MXDXL::P_TORQUE_ENABLE; value = table[addr];
			printf( " TORQUE_ENABLE          (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_LED; value = table[addr];
			printf( " LED                    (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_D_GAIN; value = table[addr];
			printf( " D_GAIN                 (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_I_GAIN; value = table[addr];
			printf( " I_GAIN                 (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_P_GAIN; value = table[addr];
			printf( " P_GAIN                 (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_RESERVED; value = table[addr];
			printf( " RESERVED               (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_GOAL_POSITION_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " GOAL_POSITION          (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_MOVING_SPEED_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " MOVING_SPEED           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_TORQUE_LIMIT_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " TORQUE_LIMIT           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_PRESENT_POSITION_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " PRESENT_POSITION       (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_PRESENT_SPEED_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " PRESENT_SPEED          (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_PRESENT_LOAD_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
			printf( " PRESENT_LOAD           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr + 1]);
			addr = MXDXL::P_PRESENT_VOLTAGE; value = table[addr];
			printf( " PRESENT_VOLTAGE        (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_PRESENT_TEMPERATURE; value = table[addr];
			printf( " PRESENT_TEMPERATURE    (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_REGISTERED_INSTRUCTION; value = table[addr];
			printf( " REGISTERED_INSTRUC     (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_MOVING; value = table[addr];
			printf( " MOVING                 (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_LOCK; value = table[addr];
			printf( " LOCK                   (R/W)[%.3d]:%5d\n", addr, value);
			addr = MXDXL::P_PUNCH_L; value = ArbotixPro::MakeWord(table[addr], table[addr + 1]);
...
void Reset(Robot::ArbotixPro *arbotixpro, int id)
{
	...
	while (1)
		{
			if (arbotixpro->WriteByte(id, MXDXL::P_RETURN_DELAY_TIME, 0, 0) == ArbotixPro::SUCCESS)
				break;
	...
	while (1)
		{
			if (arbotixpro->WriteByte(id, MXDXL::P_RETURN_LEVEL, 2, 0) == ArbotixPro::SUCCESS)
				break;
	...
	if (id != ArbotixPro::ID_CM)
		{
			double cwLimit = MXDXL::MIN_ANGLE;
			double ccwLimit = MXDXL::MAX_ANGLE;
			...
			while (1)
				{
					if (arbotixpro->WriteWord(id, MXDXL::P_CW_ANGLE_LIMIT_L, MXDXL::Angle2Value(cwLimit), 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteWord(id, MXDXL::P_CCW_ANGLE_LIMIT_L, MXDXL::Angle2Value(ccwLimit), 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteByte(id, MXDXL::P_HIGH_LIMIT_TEMPERATURE, 80, 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteByte(id, MXDXL::P_LOW_LIMIT_VOLTAGE, 60, 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteByte(id, MXDXL::P_HIGH_LIMIT_VOLTAGE, 140, 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteWord(id, MXDXL::P_MAX_TORQUE_L, MXDXL::MAX_VALUE, 0) == ArbotixPro::SUCCESS)
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteByte(id, MXDXL::P_ALARM_LED, 36, 0) == ArbotixPro::SUCCESS) // Overload, Overheat
						break;
			...
			while (1)
				{
					if (arbotixpro->WriteByte(id, MXDXL::P_ALARM_SHUTDOWN, 36, 0) == ArbotixPro::SUCCESS) // Overload, Overheat
						break;
...
{
	/*
	if(addr == MXDXL::P_RETURN_DELAY_TIME || addr == MXDXL::P_RETURN_LEVEL)
	{
	...
	*/
	...
	if (id == ArbotixPro::ID_CM)
	...
	else
		{
			...
			else if (addr == MXDXL::P_HIGH_LIMIT_TEMPERATURE
			         || addr == MXDXL::P_LOW_LIMIT_VOLTAGE
			         || addr == MXDXL::P_HIGH_LIMIT_VOLTAGE
			         || addr == MXDXL::P_ALARM_LED
			         || addr == MXDXL::P_ALARM_SHUTDOWN
			         || addr == MXDXL::P_TORQUE_ENABLE
			         || addr == MXDXL::P_LED

			         || addr == MXDXL::P_P_GAIN
			         || addr == MXDXL::P_I_GAIN
			         || addr == MXDXL::P_D_GAIN
			         || addr == MXDXL::P_RESERVED

			         || addr == MXDXL::P_LED
			         || addr == MXDXL::P_LED
			         || addr == MXDXL::P_BAUD_RATE)
				{
					res = arbotixpro->WriteByte(id, addr, value, &error);
				}
			else
	...
	//if(id == ArbotixPro::ID_CM && addr == MXDXL::P_BAUD_RATE)
